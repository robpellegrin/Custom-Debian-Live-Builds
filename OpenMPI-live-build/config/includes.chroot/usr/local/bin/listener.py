...
def listen_for_hostname():
    ...
    while True:
        ...
        # Prepare and run the mount command
        mount_command = f"sudo mount {ip_address}:/home/debian/MPI /home/user/MPI"
        try:
        ...
        try:
            ssh_keyscan_command = f"ssh-keyscan -H debian@{ip_address} >> ~/.ssh/known_hosts 2>/dev/null"
        except subprocess.CalledProcessError as e:
            print(f"An error has occurred: {e}")

        # Use sshpass to copy the SSH key, disabling strict host key checking
        try:
            sshpass_command = f"sshpass -p 'password' ssh-copy-id -o StrictHostKeyChecking=no debian@{ip_address}"
        ...
        # Use sshpass to copy the SSH key, disabling strict host key checking
        try:
            sshpass_command = f"ssh debian@{ip_address} sshpass -p 'live' ssh-copy-id -o StrictHostKeyChecking=no user@{LOCAL_IP}"
            subprocess.run(sshpass_command, shell=True, check=True)
            print("SSH key copied successfully")
        except subprocess.CalledProcessError as e:
            print(f"An error occurred: {e}")
        ...
        try:
            sshpass_command = f"ssh debian@{ip_address} 'echo user@{LOCAL_IP} >> /home/debian/allhosts'"
            subprocess.run(sshpass_command, shell=True, check=True)
        except subprocess.CalledProcessError as e:
            print(f"An error occurred: {e}")
